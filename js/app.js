(function() {
  ...
  function run() {
    if (state.currTime == 0) {
      state.break
        ...
        : ((state.currTime = times.break * 60), (state.break = true), (timer.style.color = '#50B848'), (audio.play()));
    }
    state.displayTime();
  ...
  function changeTaskTime() {
    ...
    this.textContent === '+' ? times.task += 1 : times.task < 2 ? times.task : times.task-= 1;
    state.init();
    state.displayTime();
  }
  ...
  function changeBreakTime() {
    if(state.running) return;
    this.textContent === '+' ? times.break += 1 : times.break < 2 ? times.break : times.break-= 1;
    state.init();
    state.displayTime();
  }
